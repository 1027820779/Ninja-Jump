...
window.onload = function() {
	...
	back_play= document.getElementById("back");
	back_play.addEventListener("click", move);
	function move() {
		
		if (c == true) {
			var ninja_move=document.getElementById("ninja");
			ninja_l= parseInt(window.getComputedStyle(ninja_move).left);
			//console.log(ninja_l);
			if(ninja_l==63){
			toleft();
			}
	...
	function toleft(){
		...
		if(toleftt==true){
			torightt=false;
			var ninja_move=document.getElementById("ninja");		
			move_r_timer=setInterval(function(){	
			if(gg==true){
				b+=4;
				if(b>63 && b<507){
					ninja.style.background = "url('../img/move1.gif')";
				}
				ninja_move.style.left=b+"px";
			...
			if(b==507){
				...
				window.clearInterval(move_r_timer);
				c=true;
				torightt=true;
				b=63;
			}
	...
	function toright(){
		...
		if(torightt==true){
			toleftt=false;
			var ninja_move=document.getElementById("ninja");		
        move_l_timer=setInterval(function(){
			if(gg==true){
				bb-=4;
				if(bb>63 && bb<507){
					ninja.style.background = "url('../img/move1.gif')";
				}
				ninja_move.style.left=bb+"px";
			...
			if(bb==63){
				...
				window.clearInterval(move_l_timer);
				c=true;
				toleftt=true;
				bb=507;
			}
				...
				function() {
					if (g == true) {
						...
						if ((ninja_l >= fox_l
							...
							console.log(move_r_timer,move_l_timer);
	...
	stop_play.addEventListener("click", tohta);
	function tohta() {
		if (ff == true) {
			//console.log("no");
